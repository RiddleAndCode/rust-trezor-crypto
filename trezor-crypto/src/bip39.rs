...
impl Mnemonic {
    pub fn from_phrase(phrase: &str) -> Option<Mnemonic> {
        let phrase = Self::validate_phrase(phrase)?;
        let entropy = Self::phrase_to_entropy(&phrase);
        Some(Self { phrase, entropy })
    }

    pub fn from_entropy(entropy: &[u8]) -> Option<Mnemonic> {
        if !Self::is_valid_entropy(entropy) {
            return None;
        }
        let checksum_byte = Self::calculate_checksum(entropy);
        let phrase = CString::new(
            entropy
                .iter()
                .chain(Some(&checksum_byte))
                .bits()
                .map(|bits: Bits11| Self::get_word(bits.bits() as usize).unwrap())
                .join(" "),
        )
        .unwrap();
    ...
    pub fn seed(&self, password: &str) -> [u8; SEED_LEN] {
    ...
    pub fn is_valid(phrase: &str) -> bool {
        Self::validate_phrase(phrase).is_some()
    }

    pub fn phrase(&self) -> &str {
        self.phrase.to_str().unwrap()
    }

    pub fn into_phrase(self) -> String {
        self.phrase.into_string().unwrap()
    }
    ...
    pub fn into_entropy(self) -> Vec<u8> {
        self.entropy
    }

    /// Replicating bug leading to issue https://github.com/trezor/trezor-firmware/pull/1388
    pub fn entropy_cardano(&self) -> Vec<u8> {
        Self::phrase_to_entropy_cardano(self.phrase.as_c_str())
    }
    ...
    fn validate_phrase(phrase: &str) -> Option<CString> {
        let c_string = CString::new(phrase).ok()?;
        let res = unsafe { sys::mnemonic_check(c_string.as_ptr()) };
        if res == 1 {
            Some(c_string)
        } else {
            None
    ...
    fn get_word(index: usize) -> Option<&'static str> {
        unsafe {
            let cstr = sys::mnemonic_get_word(index as i32);
            if cstr.is_null() {
                None
            } else {
                Some(CStr::from_ptr(cstr).to_str().unwrap())
            }
        }
    }

    fn calculate_checksum(entropy: &[u8]) -> u8 {
        digest::<Sha2, _>(entropy).as_bytes()[0]
    }

    fn is_valid_entropy(entropy: &[u8]) -> bool {
        let size = entropy.len() * 8;
        match size {
            128 | 192 | 256 => true,
            _ => false,
        }
...
mod tests {
    ...
    pub const TEST_VECTORS: [[&str; 4]; 24] = [
        [
